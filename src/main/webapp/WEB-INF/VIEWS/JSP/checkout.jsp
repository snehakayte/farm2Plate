...
<body class="bg-gray-900 text-white min-h-screen">
  ...
  <div class="max-w-4xl mx-auto mt-12 p-6 bg-gray-800 rounded-lg shadow-md">
    <h2 class="text-3xl font-bold text-yellow-400 mb-6">Checkout</h2>

    <!-- Cart Summary -->
    <div id="cartSummary" class="mb-8 space-y-4">
    ...
    <form id="orderForm" class="space-y-4">
      ...
      <input type="tel" name="phone" placeholder="Mobile Number" pattern="[0-9]{10}" required
             class="w-full px-4 py-2 rounded-md bg-gray-900 border border-gray-700 text-white placeholder-gray-500" />
      <textarea name="address" placeholder="Delivery Address" rows="4" required
                class="w-full px-4 py-2 rounded-md bg-gray-900 border border-gray-700 text-white placeholder-gray-500"></textarea>

      <button type="submit"
              class="bg-yellow-400 hover:bg-yellow-300 text-black font-semibold px-6 py-2 rounded-md transition duration-300">
        Place Order
      </button>
    </form>

    <p id="orderStatus" class="mt-4 hidden text-green-400 font-medium"></p>
  </div>
...
<script>
  ...
  if (cart.length === 0) {
    ...
    cart.forEach(item => {
      ...
      // Append left and right to item row
      itemEl.appendChild(leftDiv);
      itemEl.appendChild(totalP);

      // Append item row to summary
  ...
  orderForm.addEventListener("submit", async function (e) {
    ...
    try {
      const response = await fetch("<%= request.getContextPath() %>/bookorder", {
        method: "POST",
        headers: {
      ...
      orderStatus.classList.remove("hidden", "text-red-400");
      orderStatus.classList.add("text-green-400");
      ...
      orderStatus.classList.remove("hidden", "text-green-400");
      orderStatus.classList.add("text-red-400");
